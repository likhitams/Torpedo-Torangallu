...
class home_db extends CI_Model{
	...
	function getencryptPass($string, $encrypt=1) {
	    $output = false;
	    
	    $encrypt_method = "AES-256-CBC";
	    $secret_key = 'SUVEECHI';
	    $secret_iv = 'A99BC8325634E303';
...
function getGroupTripTableName($startDate, $endDate){
	...
	function getlogin($db=array(), $check=1, $pas = ""){
		...
		if($check){
			...
			$crypt = $this->getencryptPass($password);
	...
	//ALert Notification DB
	
	function getServiceAlert($det){
	    $compny = $det[0]->companyid;
		$sql = "SELECT id, ladleno ladle_no, DATE_FORMAT(STR_TO_DATE(servicedate, '%Y-%m-%d'), '%d-%m-%Y') service_date FROM ladle_master where companyid = '$compny' AND servicedate<=CURDATE() AND servicedate!='0000-00-00' order by id";
