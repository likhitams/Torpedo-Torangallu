...
class grid_db extends CI_Model{
	
	function gettable_fleetlist($db=array(), $limit = "", $cond = "", $order = "ORDER BY lastreporttime DESC"){
       ...
       $no_movement = "CONCAT(FLOOR(HOUR($times) / 24), 'd ',MOD(HOUR($times), 24), 'h ',MINUTE($times), 'm ', SECOND($times), 's')";
		...
		$sql1 = "SELECT DISTINCT(un.unitnumber) AS unitnumber, un.unitnumber AS id,
				un.unitname AS unitname,un.isengine,
				...
				un.owner AS owner,
				IFNULL(un.registration, '') AS registration,IFNULL(un.registration, '') AS oldregistration,
				s.statusdesc AS status,s.statusid,
				IF(un.altitudewidth=0,'N/A',un.altitude) AS altitude,
				  if(un.temperature=0,'N/A',un.temperature) AS temperature,
				ROUND(if(un.analog1val=0,'N/A',IFNULL(((un.analog1val*tc)/1000),'')),2) AS fuel,
				IFNULL(un.batteryno,0) batteryno, IFNULL(un.batteryno,0) oldbattery, if(un.battery_installdate='0000-00-00', '', DATE_FORMAT(un.battery_installdate,'%d-%m-%Y')) as battery_installdate, IFNULL(DATEDIFF(CURDATE(),un.battery_installdate), 0) as since, 
				...
				DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(un.reporttime)+$timezone),'%y-%m-%d %H:%i:%s') AS lastloadat,
			    ROUND(un.distance,2) AS distance,
				un.location AS locationname, ui.isalert as alerts, 
			    lm.id lmid,lm.ladleno, lm.LOAD_STATUS, lm.TAPNO, lm.SOURCE, lm.SI, lm.S, lm.TEMP,lm.GROSS_WEIGHT, lm.TARE_WEIGHT,lm.NET_WEIGHT, lm.DEST , lm.TARE_WT2,lm.NET_WT2, lm.remarks,
			    DATE_FORMAT(STR_TO_DATE(lm.LOAD_DATE, '%Y-%m-%d %H:%i:%s'), '%d-%m-%Y %H:%i:%s') LOAD_DATE, 
				DATE_FORMAT(STR_TO_DATE(lm.UNLOAD_DATE, '%Y-%m-%d %H:%i:%s'), '%d-%m-%Y %H:%i:%s') UNLOAD_DATE, IFNULL(lm.cycle,0) cycle,   
				CASE s.statusdesc                 
	                 WHEN 'Ign Off' THEN  TIMESTAMPDIFF(SECOND,reporttime,SYSDATE())
	                 WHEN 'Idle' THEN TIMESTAMPDIFF(SECOND,reporttime,SYSDATE())
				...
				$qry1
			 	FROM units un
			 	$qry2
					INNER JOIN unitrouting u ON un.unitnumber=u.unitnumber
					...
					INNER JOIN statuses s ON s.statusid=un.status 
					INNER JOIN unit_info ui ON ui.unitnumber = un.unitnumber
					LEFT JOIN ladle_master lm ON (un.registration=lm.ladleno OR un.registration=NULL)
		...
		$result = $this->db->query($sql3);
			...
			$sql2 = "SELECT DISTINCT(un.unitnumber) AS unitnumber, un.unitnumber AS id,
					un.unitname AS unitname,un.isengine,
					'N/A' as groupname,
					DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(un.lastreporttime)+$timezone),'%y-%m-%d %H:%i:%s') AS lastreporttime,
					DATE_FORMAT(FROM_UNIXTIME(un.reporttimeunix+19800),'%y-%m-%d %H:%i:%s') AS reporttime,
				...
				";
			$sql = "(".$sql1.") UNION (".$sql2.") $order $limit ";
	...
	function gettable_fleetlist5mt($db=array(), $limit = "", $cond = "", $order = "ORDER BY lastreporttime DESC"){
		$timezone= $qry1 = $qry2 = $qry3 = $qry4 = '';
		$timezone =19800;
		$role = 'c';
		$uid = 224;
	...
	function getGroupFilter($db = array(), $limit=""){
		...
		$sql = "SELECT distinct(un.unitnumber) AS unitnumber,G.groupnumber, gr.groupdesc as groupname,
				...
				FROM groupmembers G
				INNER JOIN mst_groups gr ON gr.groupnumber=G.groupnumber AND gr.is_delete=FALSE
				INNER JOIN units un ON G.unitnumber=un.unitnumber
				INNER JOIN statuses s ON s.statusid=un.status
				  INNER JOIN unit_info ui ON ui.unitnumber = un.unitnumber
				  LEFT JOIN ladle_master lm ON (un.registration=lm.ladleno OR un.registration=NULL)
	...
	function gettable_replay($db=array()){
		...
		$sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(h.reporttime)+$timezone),'%d-%m %H:%i:%s') AS reportTime, h.unitname,h.lno,
				DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(h.reporttime)+$timezone),'%Y-%m-%d %H:%i:%s') AS reportTime1, h.loadstatus,
				h.speed AS speed, h.latitude AS latitude, h.longitude AS longitude,
				 ROUND(h.distance,2) AS distance,
				  h.direction AS direction, h.location AS location, h.status as statusid, 
				 h.reporttimeunix as reporttimeunix, s.statusdesc AS STATUS, s.statusdesc AS replayStatus, s.statuscolor AS statusColor,
				  ...
				  LEFT JOIN ladle_master u ON u.ladleno=h.lno LEFT JOIN geofence_event ge ON ge.timeunix= h.reporttimeunix AND ge.lno = h.lno 
				  LEFT JOIN geofences g ON g.geofencenumber= ge.geofencenumber LEFT JOIN statuses s ON s.statusid=h.status 
				  LEFT JOIN statuses sg ON sg.statusid=ge.entry_status WHERE h.lno='$unitno' AND h.status!= 16 and (s.languageid=$language) 
        ...
        if($type != ""){
	        switch($type){
	        	...
	        	case "2": $sql = "SELECT g.geofencenumber as geofenceNumber,g.geotype as geofenceType,g.geostatus as zone,g.geofencename AS geofenceName,g.maxzonespeed AS maxSpeed,
									gr.lat1 AS latitude1,gr.lat2 AS latitude2,gr.lon1 AS longitude1,gr.lon2 AS longitude2,g.colour
									FROM geofences g
	        	...
	        	case "3": $sql = "SELECT gf.geofencenumber AS geofenceNumber,g.geotype AS geofenceType,g.geostatus AS zone,g.geofencename AS geofenceName,
									g.maxzonespeed AS maxSpeed,gf.latlon AS latlon,g.colour, g.lat, g.lon FROM geofencepoly gf 
	...
	function gettable_Track($db=array()){
		$sql = "select latlon, id, name geofenceName, id as geofenceNumber from geoployline where is_delete=0";
		$result = $this->db->query($sql);
			
		return $result->result();
	}
	
	function gettable_TrackLatest($db=array()){
		$sql = "select latlon, id, name geofenceName from geoployline where is_delete=0 ORDER BY id DESC LIMIT 1";
		$result = $this->db->query($sql);
	...
	function getFilter($columnname, $colors){
		...
		switch($columnname){            
				...
				cellClassRules: {'alert-disable': function (params) {return applyUnitClass(params,1);} }";
						break;
				case "reporttime": $filter = ", suppressFilter:true, cellRenderer: function (params) {return formatDate(params.value);}";
						break;
				case "lastreporttime": $filter = ", suppressFilter:true, cellRenderer: function (params) {return formatDate(params.value);}";
						break;
				...
				case "status":
					...
					$filter = ", filter: 'text', filterParams: { apply: true, newRowsAction: 'keep'}, cellRenderer: checkStatus
					";
				...
				case "loadstatus":
					...
					$filter = ", filter: 'text', suppressFilter:true,cellRenderer: checkLoadStatus, cellClass: 'textAlign'";
						break;
				...
				case "distance": $filter = ",  filter: 'number', filterParams: {apply: true,newRowsAction: 'keep'}, cellClass: 'textAlign'";
						break;
				case "registration": $filter = ",  filter: 'number', filterParams: {apply: true,newRowsAction: 'keep'}, cellClass: 'textAlign'";
						break;
				case "timehours": $filter = ",filter:'number', suppressFilter:true, cellRenderer: function (params) {return secondsToString(params.value);}, cellClass: 'textAlignRight'";
						break;
	...
	function getColumnFields(){
		$arr = array(
					...
					"units.registration"=>"registration",
					"units.loadstatus"=>"loadstatus"
				);
	...
	public function getUnits($db = array()){
		...
		function getSearchData($db=array(), $limit = "", $cond = "", $order = "ORDER BY lastreporttime DESC"){
    	...
    	if($role == "u"){
			 $qry1=" LEFT JOIN user_routing ur ON ur.unitrouting_id = u.routeid
					LEFT JOIN users usr ON usr.id=ur.user_id
					LEFT JOIN admin_routing ar ON ar.admin_id = usr.id OR ar.user_id = usr.id ";
    	...
    	DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(un.lastreporttime)+$timezone),'%y-%m-%d %H:%i:%s') AS lastreporttime,
    	DATE_FORMAT(FROM_UNIXTIME(un.reporttimeunix+$timezone),'%y-%m-%d %H:%i:%s') AS reporttime,
    	DATE_FORMAT(FROM_UNIXTIME(un.reporttimeunix+$timezone),'%d-%m-%y %H:%i:%s') AS reporttime1,
    	DATE_FORMAT(FROM_UNIXTIME(un.reporttimeunix+$timezone),'%m-%d-%Y %H:%i:%s') AS trackreporttime,
    	un.owner AS owner,
    	un.registration AS registration,
    	s.statusdesc AS statusdesc,s.statusid,
    	IF(un.altitudewidth=0,'N/A',un.altitude) AS altitude,
    	...
    	un.nextservice AS nextservice,
    	un.ftc as fuelCapacity,
    	IF(s.icon=1,un.direction,un.direction) AS direction,
    	lm.LOAD_STATUS AS statusColor,
    	un.status AS units_status,
    	...
    	ROUND(un.distance,2) AS distance,
    	un.location AS locationname,IFNULL(drivername,'') as driver, IFNULL(drivernumber,'') as drivermobile,
    	if(ui.routenumber=0,'NA',DATE_FORMAT(FROM_UNIXTIME(ui.routenumber+$timezone),'%d-%m-%y %H:%i:%s')) as routenumber,
		 ui.indent as indent,ui.progress as progress,ui.agent as agents,ui.location as agentloc,
    	ui.np as npedate, ui.insurence as insedate,ui.fc as fcdate,ui.amc as amcdate,ui.isalert as alerts,
    	CASE s.statusdesc
    	 WHEN 'Ign Off' THEN  TIMEDIFF((SYSDATE()+INTERVAL 0 MINUTE),reporttime)
	     WHEN 'Ign Off (Health)' THEN TIMEDIFF((SYSDATE()+INTERVAL 0 MINUTE),reporttime) 
         WHEN 'Slow/Idle' THEN TIMEDIFF((SYSDATE()+INTERVAL 0 MINUTE),reporttime) 		
         WHEN 'Moving' THEN TIMEDIFF(reporttime,reporttime) 				 
        END AS timehours,
    	IF(un.fuelwidth=0,-1,ROUND((IFNULL(un.analog1persentage,0.00)),2))
    	AS analog1persentage,
    	 lm.id lmid,lm.ladleno, lm.LOAD_STATUS, lm.TAPNO, lm.SOURCE, lm.SI, lm.S, lm.TEMP,lm.GROSS_WEIGHT, lm.TARE_WEIGHT,lm.NET_WEIGHT, lm.DEST , lm.TARE_WT2,lm.NET_WT2, lm.remarks,
			    DATE_FORMAT(STR_TO_DATE(lm.LOAD_DATE, '%Y-%m-%d %H:%i:%s'), '%d-%m-%Y %H:%i:%s') LOAD_DATE, 
				DATE_FORMAT(STR_TO_DATE(lm.UNLOAD_DATE, '%Y-%m-%d %H:%i:%s'), '%d-%m-%Y %H:%i:%s') UNLOAD_DATE, 
				DATE_FORMAT(STR_TO_DATE(lm.smstime, '%Y-%m-%d %H:%i:%s'), '%d-%m-%Y %H:%i:%s') smstime, 
				IFNULL(lm.cycle,0) cycle, IFNULL(TIMESTAMPDIFF(MINUTE,lm.TARE_DATE,now()),0) as tat
    	...
    	WHERE un.unitname='$unit' and s.languageid = '2'  
    	$qry3
    	GROUP BY u.routeid,un.unitnumber
    	";
    		
    	$sql = "(".$sql1.")   ORDER BY lastreporttime DESC $limit ";
    		echo $sql;
    	$result = $this->db->query($sql);
