	...
	<div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="newModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<!--  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> -->
					<h4 class="modal-title" id="newModalLabel">Breakdown111 Details</h4>
				</div>
				<div class="modal-body">
					...
					<form class="form-inline" id="form-validate">
						...
						<div class="form-group">
							...
							<span class="col-md-8">
								<input type="text" class="form-control driverClass" name="description" id="description">
							</span>
						</div>
	...
	<div class="page-content">
		<div class="container-fluid">
			<div class="card mt-4">
				...
				<div class="card-body">
					<h2>Ladle Maintenance Details</h2>
					<div class="table-responsived">
             <?php echo $this->session->flashdata("message"); ?>
						<div id="myGrid" class="ag-blue"></div>
					</div>
		...
		$(document).ready(function() {
			...
			$("#saveButton").click(function(evt) {
				if (v.form()) {
					...
					if (vald) {
						...
						//alert(str);
						$.post("<?php echo jquery_url(); ?>maintenance/addBreakdown", str, function(data) {
							//alert(data);
							if (parseInt(data) == 1) {
			...
			function validateForm() {
				...
				if (date == "") {
					...
					$("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Description</div>');
					return false;
				} else if (duration == "") {
			...
			$("#esaveButton").click(function(evt) {
				if (v.form()) {
					...
					if (vald) {
						...
						$.post("<?php echo jquery_url(); ?>maintenance/modifyBreakdown", str, function(data) {
							//alert(data);
							switch (data) {
								case "1":
...
var columnDefs = [
    ...
    },
    {
        ...
        cellRenderer: function(params) {
            ...
            var partialValue = params.value;
            var editButton = document.createElement('button');
            
            if (params.data.p_load >= 25) {
                // Check if both "Partial" and "Complete" counts are greater than 5
                editButton.textContent = 'RESET';
                editButton.style.backgroundColor = '#4099b6';
			    ...
			    editButton.style.marginTop = '-18px';
			    editButton.style.marginRight = '170px';
                

                editButton.addEventListener('click', function() {
                    // Handle the reset button click event here
                    var rowData = params.data;
                    // Redirect to the reset page with the ID of the selected row
                    window.location.href = 'partialreset?id=' + rowData.id;
                });
            }
            
            if (params.data.p_load >= 25 ) {
                // Both counts are greater than 5, show the value and the button in red
                cellElement.innerHTML = '<span style="color: red; font-family: Arial, sans-serif; font-size: 14px; font-weight: bold;">' + partialValue + '</span>';
                cellElement.appendChild(editButton);
            } else {
                // Display the partial value without the button in default color
                cellElement.innerHTML = partialValue;
            }
    ...
    },
    {
	   ...
	   cellRenderer: function(params) {
    var cellElement = document.createElement('div');
    var completeValue = params.value;
    var editButton = document.createElement('button');
    var ladleno = params.data.ladleno;

    function extractNumericPart(str) {
        // Assuming the identifier is in the format "TLC XX"
        return parseInt(str.split(' ')[1], 10);
    }

    var numericPart = extractNumericPart(ladleno);

    if (numericPart >= 1 && numericPart <= 29 && params.data.m_load >= 1500) {
        // Condition for "TLC 01" to "TLC 29"
        editButton.textContent = 'RESET';
        editButton.style.backgroundColor = '#4099b6';
        editButton.style.color = 'white';
        editButton.style.border = 'none';
        ...
        editButton.style.margin = '0 auto';
        editButton.style.marginTop = '-18px';
        editButton.style.marginRight = '160px';
        editButton.addEventListener('click', function() {
            // Handle the reset button click event here
            var rowData = params.data;
            // Redirect to the reset page with the ID of the selected row
            window.location.href = 'completereset?id=' + rowData.id;
        });

        cellElement.innerHTML = '<span style="color: red; font-family: Arial, sans-serif; font-size: 14px; font-weight: bold;">' + completeValue + '</span>';
        cellElement.appendChild(editButton);
    } 
    else if (numericPart >= 30 && numericPart <= 35 && params.data.m_load >= 1400) {
        // Condition for "TLC 30" to "TLC 35"
        editButton.textContent = 'RESET';
        editButton.style.backgroundColor = '#4099b6';
        editButton.style.color = 'white';
        editButton.style.border = 'none';
    ...
    else if (params.data.m_load >= 1500) {
        // Additional condition for m_load >= 1475
        cellElement.innerHTML = '<span style="color: red; font-family: Arial, sans-serif; font-size: 14px; font-weight: bold;">' + completeValue + '</span>';
        cellElement.appendChild(editButton);
    } else {
        // Default condition
        cellElement.innerHTML = completeValue;
    }
...
checkboxSelection: false
		...
		function setList() {
			...
			$.ajax({
				url: '<?php echo jquery_url() ?>maintenance/getladlemaintenancedata',
				dataType: 'json',
		...
		function deleteBreakdown() {
			...
			if (selectedRows.length) {
				...
				if (r == true) {
					...
					$.post("<?php echo base_url(); ?>maintenance/deleteBreakdown", {
						unitIds: unitIds
					}, function(data) {
						//alert(data);
		...
		function createWorksheet() {
			...
			var cellType = [],
				title = "Ladle Maintenance Details";
			var cellTypeClass = [];
