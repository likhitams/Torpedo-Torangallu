	...
	<div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="newModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<!--  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> -->
					<h4 class="modal-title" id="newModalLabel">INSPECTION DETAILS </h4>
				</div>
				...
				<div class="card-body">
					...
					<?php if ($detail[0]->userRole == 'c' || $detail[0]->userRole == 'a') { ?>
						<div class="fixed_footer">
							<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#newModal" onclick="$('.newButton').show();setDatepicker();$('.updateButton').hide();document.getElementById('divid').style.display = 'inline-flex'; $('#ladleno1')[0].selectedIndex = 0; $('#ladleno2')[0].selectedIndex = 0; $('#ladleno3')[0].selectedIndex = 0; $('#avail1')[0].selectedIndex = 0; $('#avail2')[0].selectedIndex = 0; $('#avail3')[0].selectedIndex = 0;$('#scheduledate').val('');"><i class="fa-solid fa-plus"></i> NEW</button>
...
<?php echo $jsfileone; ?>
		...
		<script type="text/javascript">
				...
				$("#saveButton").click(function(evt) {
					...
					if (v.form()) {
						...
						if (vald && ladle1 != ladle2 && ladle2 != ladle3 && ladle1 != ladle3) {
							...
							$.post("<?php echo jquery_url(); ?>maintenance/addInspec", str, function(data) {
				...
				$("#esaveButton").click(function(evt) {
					if (v.form()) {
						...
						if (vald && ladle1 != ladle2 && ladle2 != ladle3 && ladle1 != ladle3) {
							...
							$.post("<?php echo jquery_url(); ?>maintenance/modifyInsp", str, function(data) {
								//alert(data);
								switch (data) {
									case "1":
			...
			function setList() {
				...
				$.ajax({
					url: '<?php echo jquery_url() ?>maintenance/getInspDetails',
					dataType: 'json',
			...
			function deleteDump() {
				...
				if (selectedRows.length) {
					...
					if (r == true) {
						...
						$.post("<?php echo base_url(); ?>maintenance/deleteInspe", {
							unitIds: unitIds
						}, function(data) {
							//alert(data);
