...
<table>
  <tr>
    <th><strong>LOADS</strong> <span class="count gren_c monospace"><?php echo count($this->master_db->runQuerySql("select id from ladle_master where LOAD_STATUS IN (201,202,205) and cycle=1 and companyid = 95 "));?></span></th> 
      <div class="text-right f-r"><a href="<?php echo base_url()?>dashboard"><button class="btn btn-primary " type="button" >Back</button></a></div>
  </tr>

  <tr>
    <li hidden="hidden"><strong>Loads</strong> <span class="count gren_c monospace"><?php echo count($this->master_db->runQuerySql("select id from ladle_master where LOAD_STATUS IN (201,202,205) and cycle=1  and companyid = $compny "));?></span></li>
           <tr><td> <li><strong>Before Weighment</strong> <span class="count gren_c monospace"><?php echo count($this->master_db->runQuerySql("select id from ladle_master where LOAD_STATUS=201 and cycle=1   and companyid = $compny "));?></span></li></td></tr>
            <tr><td><li><strong>After  Weighment</strong> <span class="count gren_c monospace"><?php echo count($this->master_db->runQuerySql("select id from ladle_master where LOAD_STATUS IN (202,205) and cycle=1  and companyid = $compny "));?></span></li>
