...
<?php echo $jsfile;?>
 ...
 var on = true;
    ...
    function getData(){
	    ...
	    httpRequest.open('GET', '<?php echo jquery_url()?>lists/getListtenmtdata');
	    httpRequest.send();
    ...
    function refreshCountList(){
		$.get("<?php echo base_url()?>tenmt/gettenmtListCount", function(data){
			$("#refCount").html(data);
		});
	}
 ...
 // ------------------------------------------------------------------------------- //
    ...
    function listMarkers(){
    	...
    	var temp_gmarkers = gmarkers;
		    ...
		    gmarkers = [];
    	$.ajax({
	           url:  '<?php echo jquery_url()?>lists/getListtenmtdata',
	           dataType: 'text',
    ...
    function getText(status, index){
        ...
        var idlet = "";
        if(minutes > 43200 && httpResponse[index].cycle=="1"){
