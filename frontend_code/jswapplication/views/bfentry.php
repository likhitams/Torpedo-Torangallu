   ...
   <div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="newModalLabel">
      <div class="modal-dialog" role="document" >
        <div class="modal-content">
          <div class="modal-header">
          <!--  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> --> 
            <h4 class="modal-title" id="newModalLabel">BF Entry</h4>
          </div>
          <div class="modal-body">
            ...
            <form class="form-inline" id="form-validate">
             ...
             <div class="form-group">
                    <label class="col-md-4">Cast Number</label>
                    <span class="col-md-8">
                    <input type="text" class="form-control driverClass" name="castno" id="castno">
                    </span>
                  </div>
          
            <div class="form-group">
                    <label class="col-md-4">Tadpole No</label>
                    <span class="col-md-8">
                  ...
                  <div class="form-group">
                    <label class="col-md-4"> Location Id</label>
                    <span class="col-md-8">
                    <select class="form-control driverClass" name="locationid" id="locationid">
                    <option value="">Select</option>
                    <option value="Taphole1">Taphole1</option>
                    <option value="Taphole2">Taphole2</option>
                    <option value="Taphole3">Taphole3</option>
                    <option value="Taphole4">Taphole4</option>
                    </select>
                   </span>
                  </div>
                  
                   <div class="form-group">
                    <label class="col-md-4"> Load Status</label>
                    <span class="col-md-8">
                    <select class="form-control driverClass" name="loadstatus" id="loadstatus">
                    <option value="">Select</option>
	...
	<div class="main_container">
      ...
      <div id="myGrid" style="width: 100%; height: 450px ;" class="ag-blue"></div>
        ...
        <div class="fixed_footer">
        	...
        	<button type="button" class="btn btn-primary" onclick="modifyBf();setDatepicker();">MODIFY</button>
        	<button type="button" class="btn btn-dark" onclick="deleteBf();"><i class="fa-solid fa-trash"></i> DELETE</button>
	...
	$(document).ready(function(){  
	 ...
	 $("#saveButton").click(function(evt){
		if(v.form()){
			...
			if(vald){
				...
				//alert(str);
				$.post("<?php echo jquery_url();?>operations/addBf",str,function(data){
					//alert(data);
					if(parseInt(data) == 1){
	 ...
	 function validateForm(){
		 ...
		 if(castno == "" ){
				$("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Cast Number</div>');
				return false;
			}
		 else if(locationid == ""){
			 $("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Location Id</div>');
				return false;
		 }
		 else if(loadstatus == ""){
			 $("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Load Status</div>');
				return false;
		 }
		 else{
			return true;
	 ...
	 $("#esaveButton").click(function(evt){
			if(v.form()){
				...
				if(vald){
					...
					$.post("<?php echo jquery_url();?>operations/modifyBf",str,function(data){
						//alert(data);
						switch(data){
    ...
    var columnDefs = [			
        ...
        {headerName: "Tadpole Number", field: "ladleno", width:337, cellClass: 'textAlignLeft', filter: 'text',checkboxSelection: true},
        {headerName: "Cast Number", field: "castno", width:337, cellClass: 'textAlign', filter: 'text', suppressFilter:true},
        {headerName: " Location Id", field: "locationid", width:337, cellClass: 'textAlign', filter: 'text', suppressFilter:true},
        {headerName: "Load Status", field: "loadstatus", width:337, cellClass: 'textAlign', filter: 'text', suppressFilter:true, suppressSorting:true}							
	    ];
    var id = "", valatt = "";
    var gridOptions = {
 ...
 // setup the grid after the page has finished loading
   ...
   function onBtHide() {
	...
	function setList(){
	  ...
	  $.ajax({
	         url:  '<?php echo jquery_url()?>operations/getbfDetails',
	         dataType: 'json',
    ...
    function modifyBf(){
		...
		if(selectedRows.length == 0){
			$("#error-msg").html("Please select Tadpole Number to Modify");
			$("#alertbox").click();
			return;
		}
			...
			$("#oid").val(selectedRows[0].id);
	   		...
	   		$("#castno").val(selectedRows[0].castno);
	   		$("#locationid").val(selectedRows[0].locationid);
	   		$("#loadstatus").val(selectedRows[0].loadstatus);
    ...
    function deleteBf(){
		...
		if(selectedRows.length){
			...
			if(r == true){
				...
				$.post("<?php echo base_url();?>operations/deleteBf",{unitIds:unitIds},function(data){
					//alert(data);
