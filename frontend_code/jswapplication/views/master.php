...
<body class="dark-sidenav">
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="newModalLabel">Maintenance Repair Details</h4>
				...
				<div class="modal-body">
					...
					<form class="form-inline" id="form-validate">
						...
						<div class="form-group">
							...
							<span class="col-md-8">
								<input type="text" class="form-control driverClass" name="type" id="type">
							</span>
						</div>

						<div class="form-group">
							<label class="col-md-4">Sub Type of Repair</label>
							<span class="col-md-8">
								<input type="text" class="form-control driverClass" name="type_desc" id="type_desc">
							</span>
						</div>
					</form>
				</div>
				...
				<div class="modal-footer">
					...
					<!--  <button type="button" class="btn btn-dark closeButton updateButton" data-dismiss="modal" id="closeButton">Close</button>
	...
	<div class="page-content">
		<div class="container-fluid">
			...
			<div class="card mt-4">
				<div class="card-body">
					<h2>Maintenance Repair Details</h2>
					<div class="table-responsive">
						<div id="myGrid" style="width: 100%; height: 450px ;" class="ag-blue"></div>
					</div>
					<?php if ($detail[0]->userRole == 'c' || $detail[0]->userRole == 'a') { ?>
						<div class="fixed_footer">
							<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#newModal" onclick="$('.newButton').show();"><i class="fa-solid fa-plus"></i> NEW</button>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#saveButton").click(function(evt) {
				if (v.form()) {
					...
					if (vald) {
						...
						//alert(str);
						$.post("<?php echo jquery_url(); ?>operations/addDetails", str, function(data) {
							//alert(data);
							if (parseInt(data) == 1) {
			...
			function validateForm() {
				...
				if (type == "") {
					$("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Repair Type</div>');
					return false;
				} else if (type_desc == "") {
					$("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Sub Repair Type</div>');
					return false;
				} else {
					return true;
				}
		...
		function setList() {
			...
			$.ajax({
				url: '<?php echo jquery_url() ?>operations/getMaintenanceData',
				dataType: 'json',
		...
		function filterData(filterModel, data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (filterPresent && filterModel.type) {
					var name = item.type_desc;
					var allowedName = filterModel.type_desc.filter;
					var flagName = checkNumLoop(filterModel.type_desc.type, name.toLowerCase(), allowedName.toLowerCase());
		...
		function deleteDetails() {
			...
			if (selectedRows.length) {
				...
				if (r == true) {
					...
					$.post("<?php echo base_url(); ?>operations/deleteDetails", {
						unitIds: unitIds
					}, function(data) {
						//alert(data);
