   ...
   <div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="newModalLabel">
      <div class="modal-dialog" role="document" >
        <div class="modal-content">
          <div class="modal-header">
           <!--  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> -->
            <h4 class="modal-title" id="newModalLabel">Status Details</h4>
          </div>
          <div class="modal-body">
            ...
            <div class="form-group">
                    <label class="col-md-4">Ladle No</label>
                    <span class="col-md-8">
                 ...
                 <div class="form-group">
                    ...
                    <span class="col-md-8">
                    <input type="text" class="form-control driverClass" name="capacity" id="capacity">
                    </span>
                  </div>
                   ...
                   <div class="form-group">
                    ...
                    <span class="col-md-8">
                    <input type="text" class="form-control driverClass" name="supplier" id="supplier">
                    </span>
                  </div>
                
                 <div class="form-group">
                    <label class="col-md-4">Guarantee</label>
                    <span class="col-md-8">
                    <input type="text" class="form-control driverClass" name="guarantee" id="guarantee">
                    </span>
                  </div>
                  
                   <div class="form-group">
                    <label class="col-md-4">Heats</label>
                    <span class="col-md-8">
                    <input type="text" class="form-control driverClass" name="beats" id="beats">
                    </span>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <span class="col-md-8">
                    <input type="text" class="form-control driverClass" name="status" id="status">
                    </span>
                  </div>
                  
                   <div class="form-group">
                    <label class="col-md-4">Down Date</label>
                    <span class="col-md-8">
                    		<div class='input-group date date-picker1'>
                                <input type='text' class="form-control driverClass" name="downdate" id="downdate" />
                                <span class="input-group-addon" onclick="$('#downdate').focus();">
...
<div class="main_container">
       ...
       <div class="full_body"> 
        ...
        <div class="fixed_footer">
        	...
        	<button type="button" class="btn btn-primary" onclick="modifyStatus();setDatepicker();">MODIFY</button>
        	<button type="button" class="btn btn-danger" onclick="deleteStatus();">DELETE</button>
        	<button  class="btn btn-success btn-min" type="button" title="Download to Excel" onclick="convertdata();"><i class="fa fa-file-excel-o"></i></button>
...
<script src="<?php echo asset_url(); ?>js/jquery.validate.min.js"></script>
	...
	$(document).ready(function(){  
	 ...
	 $("#saveButton").click(function(evt){
		if(v.form()){
			...
			if(vald){
				...
				//alert(str);
				$.post("<?php echo jquery_url();?>maintenance/addStatus",str,function(data){
					//alert(data);
					if(parseInt(data) == 1){
	 ...
	 function validateForm(){
		 ...
		 if(downdate == "" ){
				$("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Down Date</div>');
				return false;
			}
		else if(status == ""){
			 $("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Status</div>');
				return false;
		 }
		 else if(guarantee == ""){
			 $("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Guarantee</div>');
				return false;
		 }
		 else if(supplier == ""){
			 $("#msg_box").html('<div class="alert alert-danger alert-dismissable"><button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button>Please Enter Supplier</div>');
				return false;
		 }
		 else{
			return true;
	 ...
	 $("#esaveButton").click(function(evt){
			if(v.form()){
				...
				if(vald){
					...
					$.post("<?php echo jquery_url();?>maintenance/modifyStatus",str,function(data){
						//alert(data);
						switch(data){
    ...
    var columnDefs = [			
	                  ...
	                  {headerName: "Ladle No", field: "ladleno", width:180, cellClass: 'textAlignLeft', filter: 'text',checkboxSelection: true},
	                  {headerName: "Torpedo Ladle Capacity(in tons)", field: "capacity", width:220, cellClass: 'textAlign',suppressFilter:true},
	                  {headerName: "Supplier", field: "supplier", width:180, cellClass: 'textAlign', filter: 'text', suppressFilter:true, suppressSorting:true},
	                  {headerName: "Guarantee Life", field: "guarantee", width:180, cellClass: 'textAlign', filter: 'text',suppressFilter:true},
	                  {headerName: "Life(No.of Heats)", field: "beats", width:180, cellClass: 'textAlign', filter: 'text',suppressFilter:true},
	                  {headerName: "Status", field: "status", width:180, cellClass: 'textAlign', filter: 'text',suppressFilter:true},
	                  {headerName: "Down Date", field: "downdate", width:180, cellClass: 'textAlign', filter: 'text',suppressFilter:true}
 ...
 // setup the grid after the page has finished loading
   ...
   function onBtHide() {
	...
	function setList(){
	  ...
	  $.ajax({
	         url:  '<?php echo jquery_url()?>maintenance/getLadlestatus',
	         dataType: 'json',
    ...
    function modifyStatus (){
		...
		/*else if(selectedRows[0].cycleCompleted == "1"){
			...
			$("#oid").val(selectedRows[0].id);
	   		...
	   		$("#capacity").val(selectedRows[0].capacity);
	   		$("#supplier").val(selectedRows[0].supplier);
	   		$("#guarantee").val(selectedRows[0].guarantee);
	   		$("#beats").val(selectedRows[0].beats);
	   		$("#status").val(selectedRows[0].status);
	   		$("#downdate").val(selectedRows[0].downdate);
	   		//getSubmenu();
    ...
    function deleteStatus(){
		...
		if(selectedRows.length){
			...
			if(r == true){
				...
				$.post("<?php echo base_url();?>maintenance/deleteStatus",{unitIds:unitIds},function(data){
					//alert(data);
    ...
    function createWorksheet(){
		//	Calculate cell data types and extra class names which affect formatting
          var cellType = [], title = "Torpedo Status Details";
          var cellTypeClass = [];
