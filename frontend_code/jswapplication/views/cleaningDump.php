...
<body class="dark-sidenav">
	...
	<div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="newModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<!--  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> -->
					<h4 class="modal-title" id="newModalLabel">CLEANING & DUMPING DETAILS</h4>
				</div>
				<div class="modal-body mini">
					...
					<form class="form-inline" id="form-validate">
						...
						<div class="form-group">
							...
							<span class="col-md-8">
								<div style="display:block;">
									<input type="hidden" class="form-control driverClass" name="id" id="id" maxlength="100">

									<select required name="ladleno1" id="ladleno1">
										<option value="0">Select TLC</option>
										<?php $ladle = $this->master_db->getRecords("ladle_master", array("companyid" => $detail[0]->companyid), "ladleno, id", "id asc");
										foreach ($ladle as $l) { ?>
											<option value="<?php echo $l->ladleno ?>"><?php echo $l->ladleno ?></option>
										<?php } ?>
									</select>
									<select required name="ladleno2" id="ladleno2">
										<option value="0">Select TLC</option>
										<?php $ladle = $this->master_db->getRecords("ladle_master", array("companyid" => $detail[0]->companyid), "ladleno, id", "id asc");
										foreach ($ladle as $l) { ?>
											<option value="<?php echo $l->ladleno ?>"><?php echo $l->ladleno ?></option>
									...
									<select required name="ladleno3" id="ladleno3">
										<option value="0">Select TLC</option>
										<?php $ladle = $this->master_db->getRecords("ladle_master", array("companyid" => $detail[0]->companyid), "ladleno, id", "id asc");
										foreach ($ladle as $l) { ?>
											<option value="<?php echo $l->ladleno ?>"><?php echo $l->ladleno ?></option>
						...
						<div class="form-group">
							...
							<span class="col-md-8">
								<div style="display:block;">
									<select required name="avail1" id="avail1">
										<option value="0">Select </option>
										<option value="Yes">Yes</option>
										<option value="No">No</option>
									</select>
									<select required name="avail2" id="avail2">
										<option value="0">Select </option>
										<option value="Yes">Yes</option>
										<option value="No">No</option>
									</select>
									<select required name="avail3" id="avail3">
										<option value="0">Select </option>
										<option value="Yes">Yes</option>
										<option value="No">No</option>
									</select>
						...
						<div class="form-group">
							...
							<span class="col-md-8">
								<div style="display:block;">
									<select required name="action1" id="action1">
										<option value="0">Select </option>
										<option value="Yes">Yes</option>
										<option value="No">No</option>
									</select>
									<select required name="action2" id="action2">
										<option value="0">Select </option>
										<option value="Yes">Yes</option>
										<option value="No">No</option>
									</select>
									<select required name="action3" id="action3">
										<option value="0">Select </option>
										<option value="Yes">Yes</option>
										<option value="No">No</option>
									</select>
	...
	<div class="page-content">
		<div class="container-fluid">
			...
			<div class="card mt-4">
				<div class="card-body">
					<h2>Cleaning & Dumping Details</h2>
					<div class="table-responsive">
						<div id="myGrid" style="width: 100%; height: 450px ;" class="ag-blue"></div>
					</div>
					<?php if ($detail[0]->userRole == 'c' || $detail[0]->userRole == 'a') { ?>
						<div class="fixed_footer">
							<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#newModal" onclick="$('.newButton').show();setDatepicker();$('.updateButton').hide();document.getElementById('divid').style.display = 'inline-flex'; $('#ladleno1')[0].selectedIndex = 0; $('#ladleno2')[0].selectedIndex = 0; $('#ladleno3')[0].selectedIndex = 0; $('#avail1')[0].selectedIndex = 0; $('#avail2')[0].selectedIndex = 0; $('#avail3')[0].selectedIndex = 0; $('#action1')[0].selectedIndex = 0; $('#action2')[0].selectedIndex = 0; $('#action3')[0].selectedIndex = 0;$('#scheduledate').val('');"><i class="fa-solid fa-plus"></i> NEW</button>
							<button type="button" class="btn btn-info" onclick="modifyDump();setDatepicker();"><i class="fa-solid fa-pencil"></i> MODIFY</button>
							<button type="button" class="btn btn-dark" onclick="deleteDump();"><i class="fa-solid fa-trash"></i> DELETE</button>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#saveButton").click(function(evt) {
				...
				if (v.form()) {
					...
					var ladle1 = $('#ladleno1').val();
					var ladle2 = $('#ladleno2').val();
					var ladle3 = $('#ladleno3').val();

					if (vald && ladle1 != ladle2 && ladle2 != ladle3 && ladle1 != ladle3) {
						...
						$.post("<?php echo jquery_url(); ?>maintenance/addInspecDump", str, function(data) {
						...
						alert("Ladle should not be same");
			...
			$("#esaveButton").click(function(evt) {
				if (v.form()) {
					...
					if (vald && ladle1 != ladle2 && ladle2 != ladle3 && ladle1 != ladle3) {
						...
						$.post("<?php echo jquery_url(); ?>maintenance/modifyInspDump", str, function(data) {
							//alert(data);
							switch (data) {
								case "1":
		...
		function setList() {
			...
			$.ajax({
				url: '<?php echo jquery_url() ?>maintenance/getInspDumpDetails',
				dataType: 'json',
		...
		function modifyDump() {
			...
			if (selectedRows.length == 0) {
				...
				$("#scheduledate").val(selectedRows[0].insertdate);
				$("#ladleno1").val(selectedRows[0].ladleno1);
				$("#avail1").val(selectedRows[0].availability1);
				$("#action1").val(selectedRows[0].action1);
				$("#ladleno2").val(selectedRows[0].ladleno2);
				$("#avail2").val(selectedRows[0].availability2);
				$("#action2").val(selectedRows[0].action2);
				$("#ladleno3").val(selectedRows[0].ladleno3);
				$("#avail3").val(selectedRows[0].availability3);
				$("#action3").val(selectedRows[0].action3);
		...
		function deleteDump() {
			...
			if (selectedRows.length) {
				...
				if (r == true) {
					...
					$.post("<?php echo base_url(); ?>maintenance/deleteInspeDump", {
						unitIds: unitIds
					}, function(data) {
						//alert(data);
		...
		function createWorksheet() {
			...
			var cellType = [],
				title = "Cleaning & Dumping Details";
			var cellTypeClass = [];
		...
		function exporttoExcel() {
			var title = "Cleaning & Dumping Details";
	...
	<script type="text/javascript">
		...
		function setExecutionDate() {
			...
			$j('#executiondate').datetimepicker({
				format: 'd-m-Y',
				formatDate: 'd-m-Y',
				//formatTime:'H:i',
				timepicker: false,
