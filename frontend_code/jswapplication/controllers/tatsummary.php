...
class tatsummary extends CI_Controller {
	...
	function getSummaryReport($isJson = 0){
		...
		if($res){
			foreach($res as $r){
				...
				$start_time = strtotime($r->entry_time);
				$end_time   = strtotime($r->exit_time);
				$load_time  = strtotime($r->load_time);
				
				// TAT
				$TAT_mins   = round(abs($end_time - $start_time) / 60,2);		
				
				// Iron-Zone
				$IZN_mins   = round(abs($load_time - $start_time) / 60,2);	
				
				// Steel-Zone
				$SZN_mins   = round(abs($end_time - $load_time) / 60,2);		
				...
				$row["iron_zone"]     = $this->getHoursFromMinutes($IZN_mins);
				$row["steel_zone"]    = $this->getHoursFromMinutes($SZN_mins);
				$row["tat"]           = $this->getHoursFromMinutes($TAT_mins);
	...
	{		
		...
		if($res)
		{
			...
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Iron Zone (Hrs)");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Steel Zone (Hrs)");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "TAT (Hrs)");
	...
	function getHoursFromMinutes($minutes){
		if($minutes){			
			...
			return $hrs + round($min / 60, 2);
		}else{
			return 0;
		}
	}

    public function breach_report()
    { 
		$this->data['ladle_no']   = trim(htmlspecialchars($this->input->get("unit",TRUE)));
		$this->data['start_date'] = trim(htmlspecialchars($this->input->get("start_date",TRUE)));
		$this->data['end_date']   = trim(htmlspecialchars($this->input->get("end_date",TRUE)));
		...
		$this->data['is_refill']  = trim(htmlspecialchars($this->input->get("is_refill",TRUE)));
		$this->data['mt_type']    = trim(htmlspecialchars($this->input->get("mt_type",TRUE)));
		$this->data['gf_no']      = trim(htmlspecialchars($this->input->get("gf_no",TRUE)));
		
		$db_arr = array('detail'=>$this->data['detail']);

		$this->load->view('breach_report',$this->data);       
    }
	
	function getBreachReport($isJson = 0){
		$ladle_no   = trim(htmlspecialchars($this->input->post("ladle_no",TRUE)));
		$ladle_no   = str_replace("+"," ",$ladle_no);
		$ladle_no   = is_numeric($ladle_no)?"":$ladle_no;
		...
		$gf_no      = trim(htmlspecialchars($this->input->post("gf_no",TRUE)));
		
		$rowsArr = array();
			
		$where = " where td.tat_breach_interval > 0 ";
		...
		if($mt_type != ""){
			if($mt_type == "fmt"){
				$qry .= " and ts.source_gf_id IN (select geofencenumber FROM geofences where companyid = '$company_id' and geofencename IN ('BF1', 'BF2', 'Corex1','Corex2'))";
			}else if($mt_type == "tmt"){				
				$qry .= " and ts.source_gf_id IN (select geofencenumber FROM geofences where companyid = '$company_id' and geofencename IN ('BF3','BF4'))";
			}
		}
		
		if($gf_no != ""){
			$where .= " and td.gf_id = '$gf_no' ";
		}
		
		$qry = "select td.ladleno ladle_no,gf.geofencename geofence,td.entry_dt entry_time,
		td.exit_dt exit_time,td.in_gf_interval timein_gf,td.tat_breach_interval breached_time,
		td.threshold_interval theshold_time	from tat_summary ts 
		left join tat_detail td on td.trip_id = ts.trip_id
		join geofences gf on gf.geofencenumber = td.gf_id
		$where order by ts.trip_start_dt";
		...
		if($res){
			foreach($res as $r){
				...
				$row["ladle_no"]      = $r->ladle_no;
				$row["geofence"]   	  = $r->geofence;
				$row["entry_time"]    = $r->entry_time?date("d-m-Y H:i:s",strtotime($r->entry_time)):"";
				$row["exit_time"]     = $r->exit_time?date("d-m-Y H:i:s",strtotime($r->exit_time)):"";
				$row["timein_gf"] 	  = $r->timein_gf;
				$row["theshold_time"] = $r->theshold_time;
				$row["breached_time"] = $r->breached_time;
	...
	{		
		...
		if($res)
		{
			...
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "TLC No");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Breached Geofence");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Entry Time");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Exit Time");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Time in Geofence (Min.)");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Threshold Time (Min.)");
			
			$COL = $col_arr[$cc++].$ri;					
			$sheet->setCellValue($COL, "Breached Duration (Min.)");
			...
			header('Content-Type: application/vnd.ms-excel'); //mime type
            header('Content-Disposition: attachment;filename="'.$d->getTimestamp().'-BreachedGeofenceReport.xls"'); //tell browser what's the file name
            header('Cache-Control: max-age=0'); //no cache
